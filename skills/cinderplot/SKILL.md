...
```sh
conda install -c zhou-lab cinderplot     # or: make && make install (needs cairo, zlib)
...
Input is **Newick**, not a table: `((a,b)AB,(c,d)CD)root;`. Branch lengths are
used when present, otherwise x is depth (a cladogram). `geom_tiplab()` draws the
leaf names, `geom_nodelab()` the internal ones. Layout is rectangular; omitting
`--size` fits one readable row per tip. Not implemented yet, and each says so:
`layout=circular|slanted`, and joining an external table on tip name
(`geom_tiplab(data=..., colour=...)`).
