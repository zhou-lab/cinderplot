...
- **also**: `facet_wrap(~v[, levels=][, scales=][, ncol=][, nrow=])`,
  ...
  The presets themselves are grammar-mode: in heatmap and track modes only
  `theme_*(base_line_size=)` applies (a bare preset errors), and chord and
  tree modes refuse `theme_*()` altogether.
...
**`rowcolour="colours.tsv"` colours the group names** (also `rowcolor=`): a
two-column table headed `group colour` (or `color`), one line per group, and
the name is written in that colour with a filled swatch beside it, so a lineage
reads at a glance. Groups the table does not name keep black and get no swatch;
a colour that does not parse is an error naming the row. It needs `rowgroup=`,
since without groups there is nothing to colour.
...
cinderplot 'region("chr1:202004435-202019137")
  + matrix("betas_long.tsv", x=genomic, rowgroup=" | ", rowcolour="colours.tsv",
           cluster=none, height=5)' locus.pdf
```
...
over one another; by default the crowded names are dropped and the boxes stay
(the way `genes()` keeps names from colliding). `labels=on` draws every name
regardless, `labels=off` draws none; a 3-column BED has no names to draw.
...
does not vanish under the rest. `rowgroup=` and `rowcolour=` work exactly as on
`matrix()` (the group once beside its run, a rule between runs, a coloured
name and swatch); a `name=` stands rotated at the gutter's left edge, since the
strips own the label column. `height=` is a row weight like the matrix's, and
