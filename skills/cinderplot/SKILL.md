...
or it errors naming the first missing one. The sheet is drawn only through
`rowbar="col,…"` — `rowmeta=` on its own, or with the bare `rowbar=on`, is
refused and says so. `rowmeta=` and `rowgroup=` are
...
all share one size, 9/11 of the base (9 pt at the default 11), and
`--font-size` / `theme_*(base_size=)` scale that one size. A locus figure does not rank a panel
...
its row — is shrunk toward a ~5.5pt floor until it fits. A feature name is
dropped if it will not fit even at the floor; a row label is never dropped, it
stops at the floor (never above the size asked for). Lane packing reserves the
full-size width, so a shrunk name leaves a gap rather than overlapping its
neighbour. A `signal()` panel is framed like a `matrix()` one.
