...
lengths are the datum and are never stretched, so its labels can crowd. Layout is rectangular; omitting
`--size` fits one readable row per tip.
...
belonging to three categories shows three dots. Names with no row simply get no
mark. `layout=circular|slanted` is still unimplemented and says so.
