...
- **aes**: `x`, `y` positionally, then named — `colour`/`color`/`fill`, `size`,
  `shape`, `label`, `xend`/`xmax`, `ymin`, `yend`/`ymax`, `chrom`. `shape=` maps a
...
colnames= transcripts=`, and `matrix()` also `x= bar= background= rowgroup=`. Inputs are BED/bedGraph/BEDPE/BED12/matrix TSV, tabix
