...
- **aes**: `x`, `y` positionally, then named — `colour`/`color`/`fill`, `size`,
  `shape`, `label`, `xend`/`xmax`, `yend`/`ymax`, `chrom`. `shape=` maps a
...
- **geoms**: `point jitter line smooth col histogram boxplot bar segment rect
  density tile raster hline vline abline text label`, plus
...
an arbitrary `theme()` (presets only), stacked/dodged bars, and statistical
transformations beyond binning, density and loess.
